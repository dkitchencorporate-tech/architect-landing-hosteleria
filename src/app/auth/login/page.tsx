...
function LoginForm() {
  ...
  useEffect(() => {
    // Capturar errores de redirección desde el callback
  ...
  return (
    <div className="min-h-screen bg-[#020202] flex items-center justify-center p-6 relative overflow-hidden font-sans">
      ...
      <div className="w-full max-w-[420px] bg-white/[0.02] backdrop-blur-3xl border border-white/5 rounded-3xl p-8 md:p-10 shadow-2xl relative z-10">
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          <div className="space-y-1.5">
            <label className="block text-xs font-bold text-zinc-400 uppercase tracking-wider ml-1">Email Corporativo</label>
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between items-center ml-1">
              <label className="block text-xs font-bold text-zinc-400 uppercase tracking-wider">Contraseña</label>
              <a href="#" className="text-xs text-[#FF4500] hover:text-white transition-colors">¿Olvidaste tu clave?</a>
            </div>
            <div className="relative">
              <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
          ...
          <button 
            ...
            className="w-full bg-gradient-to-r from-[#FF4500] to-orange-600 text-white font-bold py-3.5 rounded-xl mt-6 hover:opacity-90 transition-all disabled:opacity-50 shadow-[0_0_15px_rgba(255,69,0,0.3)] hover:shadow-[0_0_25px_rgba(255,69,0,0.5)] flex justify-center items-center"
          >
            {loading ? (
        ...
        <p className="text-center text-zinc-500 text-xs mt-8 font-medium">
          ¿No tienes una cuenta operativa? <Link href="/#pricing" className="text-white hover:text-[#FF4500] transition-colors underline underline-offset-4">Ver Planes</Link>
        </p>
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#020202] flex items-center justify-center text-white">Cargando ecosistema...</div>}>
      <LoginForm />
    </Suspense>
  );
}
